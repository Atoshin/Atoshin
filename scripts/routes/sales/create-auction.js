...
router.post('/', async (req, res) => {
    const { token, startingPrice, tokenAmount, duration, minIncrement} = req.body;
    if (!token || !startingPrice || !tokenAmount || !duration || !minIncrement ) {
        console.log(token,startingPrice,tokenAmount,duration,minIncrement);
        return res.status(400).json({ error: 'Missing required parameters' });
    }
    console.log(token,ethers.utils.parseEther(startingPrice.toString()),tokenAmount.toString(),duration.toString(),minIncrement.toFixed(18));
    try {
        ...
        const tx = await contract.createAuction(token,ethers.utils.parseEther(startingPrice.toFixed(18)) , tokenAmount, duration,ethers.utils.parseEther(minIncrement.toFixed(18)) , { gasLimit: gasEstimate });
